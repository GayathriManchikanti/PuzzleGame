        ...
        for (var i = 0; i < gridSize * gridSize; i++) {
            ...
            li.ondrop = (event) => {
                ...
                if (origin && dest && p) {
                    ...
                    if (isSorted(vals)) {
                        displayRandomFact(); // Display a random fact after the puzzle is solved
...
function displayRandomFact() {
    var randomFactIndex = Math.floor(Math.random() * facts.length);
    var randomFact = facts[randomFactIndex];
    var factDisplay = helper.doc('factDisplay');
    factDisplay.innerHTML = '<br>Random Fact: ' + randomFact;
}
